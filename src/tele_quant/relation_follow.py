...
from tele_quant.move_thresholds import get_significant_move_threshold
...
def _threshold_for_symbol(
    symbol: str, market: str, market_cap: float | None = None
) -> float:
    """Return the move threshold (%) applicable to this symbol.

    market_cap 있으면 move_thresholds 티어 기반 임계치 사용.
    없으면 기존 KR/US 기본값 fallback.
    """
    if market_cap is not None:
        return get_significant_move_threshold(symbol, market_cap, market)
...
class RelationFollow:
    ...
    def scan_source_moves(
        ...
        # Group edges by source symbol (source_symbol 우선, legacy source 폴백)
        sources: dict[str, list[dict[str, Any]]] = {}
        missing_src_count = 0
        for edge in active_edges:
            src = edge.get("source_symbol") or edge.get("source") or ""
        ...
        if missing_src_count:
            log.warning(
                "[relation_follow] source_symbol/source 모두 비어 있는 엣지 %d개 — DB 스키마 확인 필요",
                missing_src_count,
            )
        ...
        for src_symbol, edges in sources.items():
            ...
            market_cap = edges[0].get("market_cap")
            threshold = _threshold_for_symbol(src_symbol, src_market, market_cap)
            ...
            for edge in edges:
                tgt = edge.get("target_symbol") or edge.get("target") or ""
                if not tgt:
                    continue

...
def _fetch_market_index(market: str) -> dict[str, float]:
    ...
    try:
        ...
        for sym, name in _INDEX_SYMBOLS.get(market, []):
            data = get_ohlcv(sym, period="5d", interval="1d")
...
def _fetch_4h_data(symbol: str) -> dict[str, Any]:
    ...
    try:
        from tele_quant.stock_data_provider import get_ohlcv  # type: ignore[import-untyped]

        df = get_ohlcv(symbol, period="10d", interval="1h")
...
def _fetch_fundamentals(symbol: str) -> dict[str, Any]:
    ...
    try:
        from tele_quant.stock_data_provider import get_ticker_info  # type: ignore[import-untyped]
...
def _daily_metrics_from_df(df: Any) -> dict[str, Any]:
    if df is None or len(df) < 5:
        return {"rsi": None, "obv": "데이터 부족", "bb_pct": None, "close": None, "vol_ratio": None}
    close = df["Close"].dropna()
    volume = df["Volume"].dropna() if "Volume" in df.columns else pd.Series(0.0, index=close.index)
    if len(close) < 5:
        return {"rsi": None, "obv": "데이터 부족", "bb_pct": None, "close": None, "vol_ratio": None}
...
def _batch_daily(symbols: list[str], days: int = 25) -> dict[str, dict[str, Any]]:
    """Batch daily data with Yahoo first, then provider fallbacks when limited."""
    ...
    try:
        ...
        chunk_size = 50
        if not is_yfinance_rate_limited():
            for i in range(0, len(symbols), chunk_size):
                ...
                for sym in chunk:
                    try:
                        ...
                        result[sym] = _daily_metrics_from_df(df)
                    except Exception:
                        result[sym] = {
                            "rsi": None,
                            "obv": "데이터 부족",
        ...
        missing = [sym for sym in symbols if result.get(sym, {}).get("close") is None]
        if missing:
            log.info("[daily-alpha] fallback daily data for %d symbols", len(missing))
        for sym in missing:
            try:
                df = get_ohlcv(sym, period=f"{days}d", interval="1d")
                result[sym] = _daily_metrics_from_df(df)
            except Exception:
                result[sym] = {
                    "rsi": None,
...
def run_daily_alpha(
    ...
    # 6-B. 캘린더 정책 적용 (월요일 LONG 엄격 / 금요일 EXIT 관찰) ─────────────
    try:
        from tele_quant.calendar_score_policy import apply_policy_to_score, get_current_policy
        _cal_adj = get_current_policy()
        if _cal_adj.active:
            for p in long_picks:
                adj_score, _cal_note = apply_policy_to_score(
                    p.final_score,
                    "LONG",
                    price_change_1d=None,
                    price_change_1w=None,
                )
                if adj_score != p.final_score:
                    p.final_score = adj_score
                    p.risk_penalty += (p.final_score - adj_score)
            if _cal_adj.require_direct_evidence:
                for p in long_picks:
                    if p.direct_evidence_count < 1:
                        p.final_score = min(p.final_score, 69.9)
                        p.is_speculative = True
    except Exception as _cal_exc:
        log.debug("[daily-alpha] calendar policy adjust failed: %s", _cal_exc)

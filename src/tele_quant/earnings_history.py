...
def fetch_earnings_history(symbol: str, market: str = "", years: int = 5) -> EarningsHistory:
    ...
    try:
        ...
        divisor = 1e12 if mkt == "KR" else 1e9
        ...
        for col in reversed(cols):  # 과거→현재 순으로 YoY 계산
            ...
            rev_bn = rev / divisor if rev is not None else None
            op_bn = op_inc / divisor if op_inc is not None else None
            net_bn = net / divisor if net is not None else None

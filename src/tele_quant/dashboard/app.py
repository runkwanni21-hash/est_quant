...
.status-msg{font-size:12px;color:var(--muted);background:rgba(227,179,65,.08);border:1px solid rgba(227,179,65,.2);border-radius:6px;padding:9px 11px;margin-bottom:8px;line-height:1.6}
  ...
  <div class="content">
    ...
    <div class="page active" id="page-market">
      ...
      <div class="card">
        ...
        <div id="top-picks"><div class="empty">버튼을 누르면 워치리스트 데이터를 수집합니다. Yahoo Finance가 일시 실패해도 대시보드는 계속 동작합니다.<br><br><button class="btn sm primary" onclick="loadTopPicks(true)">오늘의 후보 불러오기</button></div></div>
      </div>
    </div>
    ...
    <div class="page" id="page-screener">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
        ...
        <button class="btn sm primary" onclick="loadScreener(true)">↺ 재스크리닝</button>
      </div>
    ...
    <div class="page" id="page-datasource">
      ...
      <div class="status-msg">
        Yahoo Finance는 무료이지만 요청 제한이 걸릴 수 있습니다. 제한 중에는 KR 가격은 pykrx/FinanceDataReader로 보완하고,
        US 가격은 Finnhub/Alpha Vantage/FMP 키가 있을 때 보완합니다. FRED/ECOS/EIA/Frankfurter는 매크로 지표 보완에 사용됩니다.
      </div>
...
let configCache = {};
const SCREENER_COOLDOWN_MS = 10 * 60 * 1000;
...
function nav(page) {
  ...
  curPage = page;
  // 페이지별 초기 로드
  if(page==='screener' && !screenerLoaded) renderScreenerIdle();
...
function setMkt(mkt) {
  ...
  else if(curPage==='screener') renderScreenerIdle();
}

function refreshCurrent() {
  if(curPage==='market') loadMarket();
  else if(curPage==='screener') loadScreener(true);
...
function dataStatusMessage(d, fallback) {
  if(d && (d.status === 'rate_limited' || d.rate_limited)) {
    return '현재 Yahoo 요청 제한 중입니다. 마지막 성공 데이터가 없으면 N/A로 표시됩니다. 잠시 후 재시도하거나 대체 데이터/API를 설정하세요.';
  }
...
function renderMacro(d) {
  if(d.error) {
    const msg = dataStatusMessage(d, 'Yahoo Finance 데이터 수신 실패. 잠시 후 새로고침하거나 네트워크 상태를 확인하세요.');
    document.getElementById('macro-body').innerHTML=`<div class="status-msg">${esc(msg)}</div><div class="empty">N/A</div>`;
    document.getElementById('index-body').innerHTML='<div class="empty">N/A</div>';
    return;
  }
  const notice = d.status && d.status !== 'ok'
    ? `<div class="status-msg">${esc(dataStatusMessage(d, 'Yahoo Finance 데이터 일부 수신 실패. 잠시 후 새로고침하거나 API/네트워크 상태를 확인하세요.'))}</div>`
    : '';
  const rows = [
  ...
  document.getElementById('macro-body').innerHTML = notice + html;
  ...
  const idxHtml = idx.map(r=>`<div class="macro-row">
    <span class="macro-label">${r.label}</span>
...
function renderTopPicksIdle() {
  document.getElementById('top-picks').innerHTML = '<div class="empty">오늘의 후보는 버튼을 누를 때 수집합니다.<br><br><button class="btn sm primary" onclick="loadTopPicks(true)">오늘의 후보 불러오기</button></div>';
}

async function loadTopPicks(force=false) {
  try {
    document.getElementById('top-picks').innerHTML = '<div class="loading"><span class="spinner"></span>스크리닝 중...</div>';
    const now = Date.now();
    let quotes, meta={};
    const cacheFresh = _screenerCache.data && _screenerCache.market===curMkt && now-_screenerCache.ts < SCREENER_COOLDOWN_MS;
    if(cacheFresh) {
      quotes = _screenerCache.data;
      meta = _screenerCache.meta || {};
      if(force) toast('최근 스크리닝 결과를 재사용합니다. Yahoo 요청량 보호를 위해 10분 쿨다운이 적용됩니다.','info');
    } else {
      const r = await fetch(`/api/screener?market=${curMkt}`);
      const d = await r.json();
      meta = d;
      quotes = d.quotes || [];
      _screenerCache = {data: quotes, market: curMkt, ts: now, meta: d};
    }
    const top = quotes.filter(q=>q.price!=null).slice(0,6);
    const notice = meta.status && meta.status !== 'ok'
      ? `<div class="status-msg">${esc(dataStatusMessage(meta, 'Yahoo Finance 데이터 일부 수신 실패. 잠시 후 새로고침하거나 네트워크 상태를 확인하세요.'))}</div>`
      : '';
    if(!top.length){ document.getElementById('top-picks').innerHTML=notice + '<div class="empty">가격 데이터가 있는 후보가 없습니다. Yahoo Finance 일시 실패 또는 네트워크 상태를 확인하세요.</div>'; return; }
    const html = notice + `<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px">
    ...
    document.getElementById('top-picks').innerHTML = `<div class="status-msg">Yahoo 요청 제한 중입니다. 잠시 후 재시도하거나 대체 데이터/API를 설정하세요.</div><div class="err-msg">${esc(e.message)}</div>`;
  }
}
...
// 스크리너
// ─────────────────────────────────────────────────────────────
function renderScreenerIdle() {
  document.getElementById('screener-meta').textContent = '버튼을 누르면 워치리스트를 수집합니다 · 10분 쿨다운';
  document.getElementById('screener-wrap').innerHTML =
    '<div class="empty">스크리너는 버튼을 누를 때만 데이터를 가져옵니다.<br>Yahoo 요청 제한이 걸리면 KR은 pykrx/FinanceDataReader, US는 설정된 대체 API를 사용합니다.<br><br><button class="btn sm primary" onclick="loadScreener(true)">스크리닝 시작</button></div>';
}
...
async function loadScreener(force=false) {
  ...
  try {
    ...
    screenerLoaded = true;
    const okCount = quotes.filter(q=>q.price!=null).length;
    document.getElementById('screener-meta').textContent=`${market} · ${okCount}/${quotes.length}개 가격 수신 · ${new Date().toLocaleTimeString('ko-KR')} 기준`;
    renderScreenerTable(quotes, meta);
  } catch(e){
    document.getElementById('screener-wrap').innerHTML=`<div class="status-msg">Yahoo 요청 제한 중입니다. 잠시 후 재시도하거나 대체 데이터/API를 설정하세요.</div><div class="loading err-msg">${esc(e.message)}</div>`;
  }
}
function renderScreenerTable(quotes, meta={}) {
  const notice = meta.status && meta.status !== 'ok'
    ? `<div class="status-msg">${esc(dataStatusMessage(meta, 'Yahoo Finance 데이터 일부 수신 실패. 잠시 후 새로고침하거나 네트워크 상태를 확인하세요.'))}</div>`
    : '';
  if(!quotes.length){ document.getElementById('screener-wrap').innerHTML=notice + '<div class="empty">결과 없음</div>'; return; }
  const rows = quotes.map(q=>`<tr onclick="goAnalysis('${esc(q.symbol)}','${esc(q.market)}')">
    <td class="sym">${esc(q.symbol)}</td>
    <td class="name-cell">${esc(q.name||q.error||'')}</td>
...
function renderAnalysis(d) {
  if(d.error && !d.price){ document.getElementById('analysis-result').innerHTML=`<div class="status-msg">${esc(dataStatusMessage(d, '종목 데이터 수신 실패. 심볼, 네트워크, Yahoo Finance 상태를 확인하세요.'))}</div><div class="err-msg">${esc(d.error)}</div>`; return; }
  const notice = d.status && d.status !== 'ok'
    ? `<div class="status-msg">${esc(dataStatusMessage(d, '일부 시장 데이터가 없어 N/A로 표시합니다.'))}</div>`
    : '';
  ...
  document.getElementById('analysis-result').innerHTML=`${notice}<div class="detail-grid">${kpiHtml}</div>${report}`;
}
...
async function loadBriefing(mkt) {
  ...
  try {
    ...
    const d = await r.json();
    const notice = d.status && d.status !== 'ok' && d.message ? `[안내] ${d.message}\n\n` : '';
    document.getElementById('briefing-body').innerHTML = esc(notice + (d.report||d.error||'결과 없음'));
  } catch(e){
...
def create_app(settings_path: str | Path | None = None) -> Any:
    ...
    _screener_api_cache: dict[str, tuple[float, dict[str, Any]]] = {}
    _screener_api_cooldown = 10 * 60.0
    ...
    async def api_macro() -> JSONResponse:
        try:
            ...
            unavailable = getattr(s, "unavailable_symbols", []) or []
            rate_limited = is_yfinance_rate_limited()
            status = "rate_limited" if rate_limited else ("partial" if unavailable else "ok")
            message = (
                yfinance_rate_limit_message()
                if rate_limited
                else (
                    "Yahoo Finance 데이터 일부 수신 실패. 잠시 후 새로고침하거나 API/네트워크 상태를 확인하세요."
                    if unavailable else ""
                )
            )
            return JSONResponse({
                ...
                "fallback_sources": getattr(s, "fallback_sources", {}) or {},
            })
        except Exception as exc:
            return JSONResponse({
                "status": "error",
                "error": str(exc),
                "message": "Yahoo Finance 데이터 수신 실패. 잠시 후 새로고침하거나 네트워크 상태를 확인하세요.",
            }, status_code=500)
    ...
    async def api_screener(market: str = "KR") -> JSONResponse:
        try:
            ...
            now = time.monotonic()
            cached = _screener_api_cache.get(cache_key)
            if cached and now - cached[0] < _screener_api_cooldown:
                payload = dict(cached[1])
                payload["cached"] = True
                payload["cooldown_seconds"] = int(_screener_api_cooldown - (now - cached[0]))
            ...
            unavailable_count = sum(1 for q in quotes if q.error or q.price is None)
            rate_limited = is_yfinance_rate_limited()
            status = "rate_limited" if rate_limited else ("partial" if unavailable_count else "ok")
            message = (
                yfinance_rate_limit_message()
                if rate_limited
                else (
            ...
            payload = {
                ...
                "fallback_count": sum(1 for q in quotes if getattr(q, "data_source", "") not in ("", "yfinance")),
                "cooldown_seconds": int(_screener_api_cooldown),
                "quotes": [_q2d(q) for q in quotes],
            }
            _screener_api_cache[cache_key] = (now, payload)
            return JSONResponse(payload)
        except Exception as exc:
            return JSONResponse({
                "status": "error",
    ...
    async def api_snapshot(ticker: str, market: str = "US") -> JSONResponse:
        try:
            ...
            rate_limited = is_yfinance_rate_limited()
            status = "rate_limited" if rate_limited and snap.close is None else ("ok" if snap.close is not None else "partial")
            return JSONResponse({
                "status": status,
                "rate_limited": rate_limited,
                "message": (
                    yfinance_rate_limit_message()
                    if rate_limited and snap.close is None else
                    "가격 데이터 없음. Yahoo Finance 일시 실패이거나 해당 심볼을 확인해야 합니다."
                    if snap.close is None else ""
                ),
            ...
            return JSONResponse({
                ...
                "symbol": ticker,
                "error": str(exc),
                "message": "종목 데이터 수신 실패. 심볼, 네트워크, Yahoo Finance 상태를 확인하세요.",
            }, status_code=500)
    ...
    async def api_briefing(market: str = "KR") -> JSONResponse:
        try:
            ...
            return JSONResponse({"status": "ok", "market": mkt, "report": report or "(결과 없음)"})
        except Exception as exc:
            return JSONResponse({
                "status": "error",
                "error": str(exc),
                "message": "브리핑 생성 중 데이터 수신 실패가 발생했습니다. 잠시 후 다시 시도하세요.",
            }, status_code=500)
...
def _q2d(q: Any) -> dict:
    return {
        ...
        "status": getattr(q, "status", ""), "data_source": getattr(q, "data_source", ""),
    }

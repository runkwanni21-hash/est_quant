...
async function doAnalysis() {
  const raw = document.getElementById('a-ticker').value.trim();
  const sym = raw.toUpperCase();
  if(!sym) return;
  // .KS/.KQ 티커 → 자동으로 KR 시장 선택
  const selMkt = document.getElementById('a-mkt');
  if(sym.endsWith('.KS') || sym.endsWith('.KQ')) selMkt.value = 'KR';
  ...
  try {
    const r = await fetch(`/api/snapshot/${encodeURIComponent(raw)}?market=${mkt}`);
    const d = await r.json();
    // 서버가 실제 시장 판별 후 반환하면 드롭다운을 동기화
    if(d.market === 'KR') selMkt.value = 'KR';
    else if(d.market === 'US') selMkt.value = 'US';
    renderAnalysis(d);
  } catch(e){
...
def _build_alias_map() -> dict[str, str]:
    """ticker_aliases.yml의 aliases → symbol 역방향 맵 반환."""
    try:
        import yaml
        aliases_path = Path(__file__).parent.parent.parent.parent / "config" / "ticker_aliases.yml"
        if not aliases_path.exists():
            aliases_path = Path("config/ticker_aliases.yml")
        ...
        for stock in data.get("stocks", []):
            ...
            name = stock.get("name", "")
            if name:
                result[name.lower()] = sym
            # aliases 목록
            for alias in stock.get("aliases", []):
                result[str(alias).lower()] = sym
        return result
    except Exception:
        return {}
...
def _resolve_input_ticker(raw: str) -> tuple[str, str]:
    """사용자 입력 → (ticker, market) 자동 변환.

    - 한글/영문 회사명 → YAML alias 맵 조회
    - 순수 숫자 6자리 → KR 티커 (.KS 자동 추가)
    - .KS/.KQ 포함 → KR
    """
    raw = raw.strip()
    # alias 맵 조회 (대소문자 무시)
    mapped = _ALIAS_MAP.get(raw.lower())
    if mapped:
        market = "KR" if mapped.endswith((".KS", ".KQ")) else "US"
        return mapped, market
    # 순수 숫자 6자리 → 005930 형태이면 KR
...
def create_app(settings_path: str | Path | None = None) -> Any:
    ...
    async def api_snapshot(ticker: str, market: str = "") -> JSONResponse:
        import asyncio
        ...
        try:
            ...
            # 회사명·별칭 → 티커 자동 변환
            # alias 맵에서 감지한 시장이 있으면 query param보다 우선
            resolved_ticker, resolved_market = _resolve_input_ticker(ticker)
            if market and ticker.upper() == resolved_ticker:
                # 사용자가 이미 정확한 티커를 입력한 경우에만 query param 존중
                resolved_market = market.upper()

            def _run() -> JSONResponse:
                store = Store(_P(cfg.sqlite_path))
                snap = build_stock_snapshot(resolved_ticker, resolved_market, store=store, deep=False)
                return JSONResponse({

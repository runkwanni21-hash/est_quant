...
def _fetch_one(symbol: str) -> QuickQuote:
    ...
    try:
        from tele_quant.stock_data_provider import get_ohlcv, get_ohlcv_status, get_ticker_info
        ...
        st = get_ohlcv_status(symbol, period=_PRICE_PERIOD, interval="1d")
        q.status = st.status
        q.data_source = st.source
        if df is None or df.empty:
            q.error = st.message or "데이터 없음"
            return q
...
def run_screener(
    ...
    try:
        ...
        seen: set[Any] = set()
        try:
            iterator = as_completed(futs, timeout=timeout)
            for fut in iterator:
                seen.add(fut)
                try:
                    ...
                    sym = futs[fut]
                    log.debug("[screener] %s timeout/error: %s", sym, exc)
                    results.append(QuickQuote(symbol=sym, error="데이터 수신 실패"))
        ...
        for fut, sym in futs.items():
            if fut in seen:
                continue
            fut.cancel()
            results.append(QuickQuote(symbol=sym, error="데이터 수신 시간 초과"))
    finally:
        pool.shutdown(wait=False, cancel_futures=True)

    unavailable = [q for q in results if q.error or q.price is None]
    if unavailable:
        log.info(r"\[market-data] %d symbols unavailable; dashboard continues", len(unavailable))

    results.sort(key=lambda x: (x.price is not None, x.score), reverse=True)
    return results

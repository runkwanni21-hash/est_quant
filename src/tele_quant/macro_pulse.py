...
def _fetch_one(ticker: str, as_bp: bool = False) -> tuple[float | None, float | None, str, str]:
    """최신 종가 + 변화 반환.
    as_bp=True  → 절대 변화(베이시스포인트 단위, 금리용)
    as_bp=False → 퍼센트 변화율(일반 가격용)
    """
    try:
        from tele_quant.stock_data_provider import get_ohlcv, get_ohlcv_status

        df = get_ohlcv(ticker, period="5d", interval="1d")
        st = get_ohlcv_status(ticker, period="5d", interval="1d")
        if df is None or df.empty or "Close" not in df.columns:
            return None, None, st.status, st.source
        ...
        return price, chg, st.status, st.source
...
def fetch_macro_snapshot() -> MacroSnapshot:
    ...
    with ThreadPoolExecutor(max_workers=min(4, len(keys))) as pool:
        ...
        for fut in as_completed(futs):
            k = futs[fut]
            try:
                price, chg, status, source = fut.result()
            except Exception:
                price, chg, status, source = None, None, "error", ""
            if price is None and chg is None:
                snap.unavailable_symbols.append(_MACRO_TICKERS[k])
            if status == "rate_limited":
                snap.rate_limited_symbols.append(_MACRO_TICKERS[k])
            if source and source != "yfinance":
                snap.fallback_sources[_MACRO_TICKERS[k]] = source
    ...
    _apply_external_fallbacks(snap)
    filled_after_fallback = {
        "CL=F": snap.wti_price,
        "^TNX": snap.us10y,
        "KRW=X": snap.usd_krw,
        "^VIX": snap.vix,
        "GC=F": snap.gold_price,
        "^GSPC": snap.sp500_chg,
        "^KS11": snap.kospi_chg,
        "DX-Y.NYB": snap.dxy,
    }
    snap.unavailable_symbols = [
        sym for sym in snap.unavailable_symbols
        if filled_after_fallback.get(sym) is None
    ]
    snap.regime = macro_regime(snap)
    snap.interpretations = interpret_macro(snap)
    if snap.unavailable_symbols:
        log.info(
            r"\[market-data] %d macro symbols unavailable; dashboard continues",
            len(snap.unavailable_symbols),
        )
    return snap


def _apply_external_fallbacks(snap: MacroSnapshot) -> None:
    """Fill missing macro values from official/free APIs when configured."""
    try:
    ...
    try:
        if getattr(cfg, "fred_enabled", True) and getattr(cfg, "fred_api_key", ""):
            from tele_quant.external_indicators import fetch_fred_series

            fred = fetch_fred_series(
                getattr(cfg, "fred_api_key", ""),
                ["DGS10", "VIXCLS", "DTWEXBGS"],
                getattr(cfg, "fred_timeout_seconds", 12.0),
            )
            if snap.us10y is None and fred.get("DGS10") is not None:
                snap.us10y = fred["DGS10"]
                snap.fallback_sources["^TNX"] = "fred"
            if snap.vix is None and fred.get("VIXCLS") is not None:
                snap.vix = fred["VIXCLS"]
                snap.fallback_sources["^VIX"] = "fred"
            if snap.dxy is None and fred.get("DTWEXBGS") is not None:
                snap.dxy = fred["DTWEXBGS"]
                snap.fallback_sources["DX-Y.NYB"] = "fred"
    except Exception as exc:
        log.debug("[macro_pulse] FRED fallback failed: %s", exc)

    try:
        if getattr(cfg, "eia_enabled", True) and getattr(cfg, "eia_api_key", ""):
            from tele_quant.external_indicators import fetch_eia_energy

            energy = fetch_eia_energy(
                getattr(cfg, "eia_api_key", ""),
                getattr(cfg, "eia_timeout_seconds", 10.0),
            )
            if snap.wti_price is None and energy.get("wti") is not None:
                snap.wti_price = energy["wti"]
                snap.fallback_sources["CL=F"] = "eia"
    except Exception as exc:
        log.debug("[macro_pulse] EIA fallback failed: %s", exc)

    try:
        if snap.usd_krw is None and getattr(cfg, "ecos_enabled", True) and getattr(cfg, "ecos_api_key", ""):
            from tele_quant.ecos_client import fetch_ecos_series

            ecos = fetch_ecos_series(
                getattr(cfg, "ecos_api_key", ""),
                timeout=getattr(cfg, "ecos_timeout_seconds", 12.0),
            )
            if ecos.get("731Y003") is not None:
                snap.usd_krw = ecos["731Y003"]
                snap.fallback_sources["KRW=X"] = "ecos"
    except Exception as exc:
        log.debug("[macro_pulse] ECOS fallback failed: %s", exc)

    try:
        if snap.usd_krw is None and getattr(cfg, "frankfurter_enabled", True):
            from tele_quant.external_indicators import fetch_exchange_rates

            rates = fetch_exchange_rates(
                base="USD",
                targets="KRW",
                timeout=getattr(cfg, "frankfurter_timeout_seconds", 8.0),
            )
            if rates.get("KRW") is not None:
                snap.usd_krw = rates["KRW"]
                snap.fallback_sources["KRW=X"] = "frankfurter"
    except Exception as exc:
        log.debug("[macro_pulse] Frankfurter fallback failed: %s", exc)

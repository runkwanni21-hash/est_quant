...
class FundamentalSnapshot:
    ...
    sector: str
    fetched_at: datetime
    industry: str = ""   # yfinance industry (sector보다 세밀)
...
def fetch_fundamentals(symbol: str, market: str = "", sector: str = "") -> FundamentalSnapshot:
    ...
    try:
        ...
        info: dict[str, Any] = get_ticker_info(symbol)
        ...
        # 섹터/산업 (yfinance 우선, 파라미터 fallback)
        if not snap.sector:
            snap.sector = info.get("sector") or ""
        snap.industry = info.get("industry") or ""

        # 배당 — KR은 DART 우선, fallback=yfinance (소수형: 0.0054=0.54%; 0.5이상=오류)
        dy = info.get("dividendYield")
        yf_div = float(dy) * 100 if dy and 0 < float(dy) < 0.5 else None
        if market == "KR":
            try:
                from tele_quant.opendart_client import fetch_dart_dividend
                from tele_quant.settings import Settings as _Settings
                _dart_key = getattr(_Settings(), "opendart_api_key", "") or ""
                dart_div = fetch_dart_dividend(symbol, _dart_key) if _dart_key else None
                snap.dividend_yield = dart_div if dart_div is not None else yf_div
            except Exception:
                snap.dividend_yield = yf_div
        else:
            snap.dividend_yield = yf_div

        # 심화 밸류에이션
        ev_eb = info.get("enterpriseToEbitda")
        snap.ev_to_ebitda = float(ev_eb) if ev_eb and 0 < float(ev_eb) < 1000 else None

        fcf = info.get("freeCashflow")
        mc_cap = info.get("marketCap")
        if fcf is not None and mc_cap and float(mc_cap) > 0:
            snap.fcf_yield = float(fcf) / float(mc_cap) * 100

        gm = info.get("grossMargins")
        snap.gross_margin = float(gm) * 100 if gm is not None else None

        cr = info.get("currentRatio")
        snap.current_ratio = float(cr) if cr and float(cr) > 0 else None

        peg = info.get("pegRatio")
        snap.peg_ratio = float(peg) if peg and 0 < float(peg) < 100 else None

        # 애널리스트 컨센서스
        tgt = info.get("targetMeanPrice")
        snap.analyst_target = float(tgt) if tgt and float(tgt) > 0 else None

        n_op = info.get("numberOfAnalystOpinions")
        snap.analyst_count = int(n_op) if n_op else None

        rec = info.get("recommendationMean")
        snap.analyst_rec_mean = float(rec) if rec else None
        ...
        # 애널리스트 목표가 괴리율 (current_price 확정 후)
        if snap.analyst_target and snap.current_price and snap.current_price > 0:
            snap.analyst_target_upside = (
                (snap.analyst_target - snap.current_price) / snap.current_price * 100
            )
    ...
    # 재무 이상치 탐지
    try:
        from tele_quant.financial_sanity import check_financial_sanity, format_sanity_note
        sr = check_financial_sanity(
        ...
        snap.data_confidence = sr.confidence
        snap.sanity_note = format_sanity_note(sr)
    except Exception as exc:
        log.debug("[fundamentals] sanity check failed %s: %s", symbol, exc)
...
def score_fundamentals(snap: FundamentalSnapshot, side: str = "LONG") -> tuple[float, str]:
    ...
    pe = snap.pe_trailing or snap.pe_forward
    _pe_label = "PER(F)" if snap.pe_trailing is None and snap.pe_forward is not None else "PER"
    pb = snap.pb
    roe = snap.roe
    ...
    if side == "LONG":
        ...
        if pe is not None and pe > 0:
            if pe < 8:
                score += 20
                parts.append(f"{_pe_label} {pe:.1f} 극저평가")
            elif pe < 15:
                score += 12
                parts.append(f"{_pe_label} {pe:.1f} 저평가")
            elif pe < 25:
                score += 5
            elif pe > 50:
                score -= 10
                parts.append(f"{_pe_label} {pe:.1f} 고평가")
        ...
        if pe is not None and pe > 0:
            if pe > 80:
                score += 25
                parts.append(f"{_pe_label} {pe:.1f} 극고평가")
            elif pe > 50:
                score += 15
                parts.append(f"{_pe_label} {pe:.1f} 고평가")
            elif pe < 10:

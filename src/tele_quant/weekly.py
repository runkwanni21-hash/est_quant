...
def _ret_bar(pct: float, width: int = 8) -> str:
    """수익률을 시각적 바로 표현. -10%~+10% 기준 width 블록."""
    clamped = max(-10.0, min(10.0, pct))
    filled = round(abs(clamped) / 10.0 * width)
    if pct >= 0:
        return "▓" * filled + "░" * (width - filled)
    return "░" * (width - filled) + "▓" * filled


def _perf_icon(ret_pct: float, win: bool) -> str:
    if win and ret_pct >= 5:
        return "🚀"
    if win and ret_pct >= 2:
        return "✅"
    if win:
        return "🟢"
    if ret_pct <= -5:
...
def build_long_short_signal_review_section(
    ...
    """80점 이상 첫 신호 LONG/SHORT 가격기반 성과 스코어보드.

    개인 투자자 관점: 한눈에 승/패, 등락률, 신호 시점을 파악할 수 있도록
    스코어보드 형식으로 출력한다.
    """
    now = datetime.now(UTC)
    since_kst = since.astimezone(_KST_OFFSET).strftime("%m/%d")
    until_kst = (until or now).astimezone(_KST_OFFSET).strftime("%m/%d")
    lines: list[str] = [
        f"📊 주간 성과 스코어보드  {since_kst} ~ {until_kst}",
        "─" * 32,
        "신호가 기준 가상 등락 — 실제 매매 수익 아님",
        "",
    ]

    all_evaluable: list[dict] = []

    for side_label, side_code in [("LONG", "LONG"), ("SHORT/회피", "SHORT")]:
        try:
        ...
        for row in rows:
            ...
            if side_code == "LONG":
                ...
                win = review_price < signal_price
            evaluable.append(
                {**row, "_review_price": review_price, "_ret_pct": ret_pct,
                 "_win": win, "_side": side_code, "_market": market}
            )
        ...
        # ── KR / US 분리 출력 ──────────────────────────────────────────────
        for mkt_label, mkt_key in [("🇰🇷 KR", "KR"), ("🇺🇸 US", "US")]:
            mkt_items = [e for e in evaluable if e["_market"] == mkt_key]
            if not mkt_items:
                continue

            wins = [e for e in mkt_items if e["_win"]]
            avg_ret = sum(e["_ret_pct"] for e in mkt_items) / len(mkt_items)
            win_rate = len(wins) / len(mkt_items) * 100

            lines.append(
                f"{mkt_label}  {side_label}  "
                f"승률 {len(wins)}/{len(mkt_items)} ({win_rate:.0f}%)  "
                f"평균 {avg_ret:+.1f}%"
            )

            # 등락 순서로 정렬 (좋은 것 위)
            sorted_items = sorted(mkt_items, key=lambda x: -x["_ret_pct"])
            for e in sorted_items[:8]:
                sym = e.get("symbol", "?")
                ...
                if created_at:
                    ...
                    if isinstance(created_at, datetime):
                        if created_at.tzinfo is None:
                            created_at = created_at.replace(tzinfo=UTC)
                        signal_date = created_at.astimezone(_KST_OFFSET).strftime("%m/%d")

                icon = _perf_icon(ret_pct, win)
                bar = _ret_bar(ret_pct)
                price_str = ""
                if s_price and r_price:
                    price_str = (
                        f"{_fmt_price(s_price, mkt_key)}→{_fmt_price(r_price, mkt_key)}"
                    )
                lines.append(
                    f"  {icon} {name_short:<8} {score:.0f}pt  "
                    f"{ret_pct:+5.1f}%  [{bar}]"
                    + (f"  {signal_date}" if signal_date else "")
                )
                if price_str:
    ...
    # ── 전체 요약 ─────────────────────────────────────────────────────────
    if all_evaluable:
        total_wins = [e for e in all_evaluable if e["_win"]]
        total_avg = sum(e["_ret_pct"] for e in all_evaluable) / len(all_evaluable)
        best = max(all_evaluable, key=lambda x: x["_ret_pct"])
        worst = min(all_evaluable, key=lambda x: x["_ret_pct"])
        best_name = best.get("name") or best.get("symbol", "?")
        worst_name = worst.get("name") or worst.get("symbol", "?")
        lines.append("─" * 32)
        lines.append(
            f"종합  신호 {len(all_evaluable)}개  "
            f"승률 {len(total_wins)}/{len(all_evaluable)} "
            f"({len(total_wins)/len(all_evaluable)*100:.0f}%)  "
            f"평균 {total_avg:+.1f}%"
        )
        lines.append(f"  최고: {best_name}  {best['_ret_pct']:+.1f}%")
        lines.append(f"  최악: {worst_name}  {worst['_ret_pct']:+.1f}%")
    else:
        lines.append("이번 주 80점 이상 신호 없음 (또는 가격 확인 불가)")

...
# pydantic-settings는 os.environ을 채우지 않으므로
# pykrx 등 os.environ을 직접 읽는 라이브러리를 위해 명시적으로 로드.
...
class Settings(BaseSettings):
    ...
    # 대시보드 마스터키 (소유자용, 서버 재시작 전까지 무제한 세션)
    dashboard_master_key: str = ""
    # 대시보드 일반 비밀번호 (사용자용, 24시간 세션)
    ...
    # US equity price fallback providers used when Yahoo Finance is rate-limited.
    ...
    @field_validator("opendart_api_key", mode="before")
    @classmethod
    def dart_api_key_alias(cls, value: Any) -> Any:
        """Accept legacy DART_API_KEY as a fallback for OPENDART_API_KEY."""
        if not value:
            legacy = os.environ.get("DART_API_KEY", "").strip()
            if legacy:
                return legacy
        return value
    ...
    @classmethod
    def alpha_vantage_api_key_alias(cls, value: Any) -> Any:
        """Accept legacy ALPHA_VANTAGE_API_KEY as a fallback for ALPHAVANTAGE_API_KEY."""
        if not value:
    ...
    def validate_minimum(self, mode: str = "full") -> list[str]:
        """Return a list of configuration problems for the given operation mode.

        Modes:
          ``dashboard`` — no Telegram credentials required (yfinance-only preview).
          ``send``      — requires bot token + target chat id when send_mode=bot.
          ``collect``   — requires Telegram API_ID/HASH and source chats.
          ``full``      — all checks (legacy default; used by the full pipeline).
        """
        issues: list[str] = []
        ...
        if mode in ("send", "full") and self.telegram_send_mode == "bot":

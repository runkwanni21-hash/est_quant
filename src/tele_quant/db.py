...
-- ── Weekly Cycle Advisory ──────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS advisory_cycle_messages (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    created_at TEXT NOT NULL,
    slot TEXT NOT NULL,
    ...
    UNIQUE(slot, body_hash)
);

CREATE INDEX IF NOT EXISTS idx_advisory_cycle_created_at ON advisory_cycle_messages(created_at);
CREATE INDEX IF NOT EXISTS idx_advisory_cycle_slot ON advisory_cycle_messages(slot);

CREATE TABLE IF NOT EXISTS advisory_recommendation_log (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    created_at TEXT NOT NULL,
    slot TEXT NOT NULL,
    symbol TEXT NOT NULL,
    ...
    source_event_symbol TEXT NOT NULL DEFAULT '',
    source_event_reason TEXT NOT NULL DEFAULT '',
    relation_type TEXT NOT NULL DEFAULT '',
    direction TEXT NOT NULL DEFAULT '',
    expected_lag_hours INTEGER DEFAULT 24,
    entry_ref_price REAL,
    invalidation_price REAL,
    target_zone_price REAL,
    horizon TEXT NOT NULL DEFAULT '1D~1M',
    ...
    status TEXT NOT NULL DEFAULT 'OPEN',
    reviewed_at TEXT,
    review_return_pct REAL,
    review_note TEXT NOT NULL DEFAULT '',
    UNIQUE(slot, symbol, side, source_event_symbol, created_at)
);

CREATE INDEX IF NOT EXISTS idx_advisory_rec_created_at ON advisory_recommendation_log(created_at);
CREATE INDEX IF NOT EXISTS idx_advisory_rec_symbol ON advisory_recommendation_log(symbol);
CREATE INDEX IF NOT EXISTS idx_advisory_rec_status ON advisory_recommendation_log(status);
CREATE INDEX IF NOT EXISTS idx_advisory_rec_slot ON advisory_recommendation_log(slot);
...
CREATE INDEX IF NOT EXISTS idx_ticker_universe_market ON ticker_universe(market);
CREATE INDEX IF NOT EXISTS idx_ticker_universe_sector ON ticker_universe(sector_id);
CREATE INDEX IF NOT EXISTS idx_ticker_universe_active ON ticker_universe(is_active);
"""
...
class Store:
    ...
    def save_cycle_message(
        ...
        """advisory_cycle_messages에 저장. 중복이면 False 반환."""
        from tele_quant.textutil import content_hash
        h = content_hash(body)
        now = utc_now().isoformat()
        with self.connect() as conn:
            try:
                conn.execute(
                    """INSERT INTO advisory_cycle_messages
                       (created_at, slot, market, title, body_hash, sent, telegram_message_id)
                       VALUES (?,?,?,?,?,?,?)""",
                    (now, slot, market, title, h, int(sent), telegram_message_id),
                )
                conn.commit()
                return True
            except sqlite3.IntegrityError:
    ...
    def recent_cycle_messages(
        self,
        since: datetime | None = None,
        slot: str | None = None,
        limit: int = 20,
    ) -> list[dict]:
        since_str = (since or (utc_now() - timedelta(days=7))).isoformat()
        with self.connect() as conn:
            if slot:
                rows = conn.execute(
                    "SELECT * FROM advisory_cycle_messages WHERE created_at>=? AND slot=? ORDER BY created_at DESC LIMIT ?",
                    (since_str, slot, limit),
                ).fetchall()
            else:
                rows = conn.execute(
                    "SELECT * FROM advisory_cycle_messages WHERE created_at>=? ORDER BY created_at DESC LIMIT ?",
                    (since_str, limit),
                ).fetchall()
        return [dict(r) for r in rows]

    def save_recommendation_log(self, entries: list[dict]) -> int:
        """advisory_recommendation_log에 저장. 중복은 무시."""
        now = utc_now().isoformat()
        count = 0
        with self.connect() as conn:
            for e in entries:
                try:
                    conn.execute(
                        """INSERT OR IGNORE INTO advisory_recommendation_log
                           (created_at, slot, symbol, name, market, side, score, confidence,
                            source_event_symbol, source_event_reason, relation_type, direction,
                            expected_lag_hours, entry_ref_price, invalidation_price, target_zone_price,
                            horizon, thesis, evidence_json, technical_json, policy_note, status)
                           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                        (
                            ...
                            e.get("name", ""),
                            e.get("market", ""),
                            e.get("side", "WATCH_ONLY"),
                            float(e.get("score") or 0),
                            e.get("confidence", ""),
                            e.get("source_event_symbol", ""),
                            e.get("source_event_reason", ""),
                            e.get("relation_type", ""),
                            e.get("direction", ""),
                            int(e.get("expected_lag_hours") or 24),
                            e.get("entry_ref_price"),
                            e.get("invalidation_price"),
                            e.get("target_zone_price"),
                            e.get("horizon", "1D~1M"),
                            e.get("thesis", ""),
                            json.dumps(e.get("evidence_json") or [], ensure_ascii=False),
                            json.dumps(e.get("technical_json") or {}, ensure_ascii=False),
                            e.get("policy_note", ""),
                            e.get("status", "OPEN"),
                        ),
                    )
    ...
    def recent_recommendation_log(
        ...
        with self.connect() as conn:
            rows = conn.execute(
                f"SELECT * FROM advisory_recommendation_log WHERE {where} ORDER BY created_at DESC LIMIT ?",
                (*params, limit),
            ).fetchall()
        return [dict(r) for r in rows]
    ...
    def update_recommendation_review(
        ...
        with self.connect() as conn:
            conn.execute(
                """UPDATE advisory_recommendation_log
                   SET status=?, reviewed_at=?, review_return_pct=?, review_note=?
                   WHERE id=?""",
                (status, now, review_return_pct, review_note, rec_id),
            )
            conn.commit()

    # ── ticker_universe ───────────────────────────────────────────────────────

    def upsert_ticker_universe(self, rows: list[dict]) -> int:
        """ticker_universe 일괄 upsert. rows: symbol 필수 나머지 optional."""
        if not rows:
            return 0
        now = utc_now().isoformat()
        count = 0
            ...
            for r in rows:
                conn.execute(
                    """INSERT INTO ticker_universe
                       (symbol, name_ko, name_en, market, exchange, sector_id,
                        sector_label, industry, market_cap_usd, is_active, is_etf, updated_at)
                       VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
                       ON CONFLICT(symbol) DO UPDATE SET
                         ...
                         sector_id=CASE WHEN excluded.sector_id!='' THEN excluded.sector_id ELSE sector_id END,
                         sector_label=CASE WHEN excluded.sector_label!='' THEN excluded.sector_label ELSE sector_label END,
    ...
    def get_ticker_name(self, symbol: str) -> str:
        """ticker_universe에서 한국명 우선, 없으면 영문명, 없으면 symbol 반환."""
        with self.connect() as conn:
            row = conn.execute(
                "SELECT name_ko, name_en FROM ticker_universe WHERE symbol=?", (symbol,)
            ).fetchone()
        if row:
            return row["name_ko"] or row["name_en"] or symbol
        return symbol

    def get_ticker_sector(self, symbol: str) -> str:
        """ticker_universe에서 sector_id 반환. 없으면 빈 문자열."""
        with self.connect() as conn:
            row = conn.execute(
                "SELECT sector_id FROM ticker_universe WHERE symbol=?", (symbol,)
            ).fetchone()
        return row["sector_id"] if row else ""
    ...
    def get_universe_symbols(
        ...
        if exclude_etf:
            where.append("is_etf=0")
        sql = "SELECT symbol FROM ticker_universe"
        if where:
            sql += " WHERE " + " AND ".join(where)
        sql += " ORDER BY market_cap_usd DESC NULLS LAST LIMIT ?"
        params.append(limit)
        with self.connect() as conn:
            rows = conn.execute(sql, params).fetchall()
        return [r["symbol"] for r in rows]

    def get_universe_count(self, market: str = "") -> dict[str, int]:
        """universe 통계: {market: count, ...}"""
        with self.connect() as conn:
            rows = conn.execute(
                "SELECT market, COUNT(*) as cnt FROM ticker_universe "
                "WHERE is_active=1 GROUP BY market"
            ).fetchall()
        result = {r["market"]: r["cnt"] for r in rows}

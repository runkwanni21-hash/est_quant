...
# Map of human-readable name → canonical environment variable name.
# Values are NEVER read here; only presence is checked.
_PROVIDER_ENV_VARS: dict[str, str] = {
    "fred": "FRED_API_KEY",
    "finnhub": "FINNHUB_API_KEY",
    "fmp": "FMP_API_KEY",
    # canonical: ALPHAVANTAGE_API_KEY  (legacy alias: ALPHA_VANTAGE_API_KEY)
    "alphavantage": "ALPHAVANTAGE_API_KEY",
    # canonical: OPENDART_API_KEY  (legacy alias: DART_API_KEY)
...
# Legacy / alternative env var names accepted as fallback aliases.
# Keys without a key-in-_PROVIDER_ENV_VARS and without an alias are simply absent.
_PROVIDER_ENV_ALIASES: dict[str, str] = {
...
def available_providers(load_external: bool = True) -> dict[str, bool]:
    ...
    for name, env_var in _PROVIDER_ENV_VARS.items():
        ...
        if not val:
            # check legacy alias (e.g. ALPHA_VANTAGE_API_KEY, DART_API_KEY)
            alias = _PROVIDER_ENV_ALIASES.get(name, "")
            if alias:

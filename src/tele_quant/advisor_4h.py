...
def run_4h_advisory(
    ...
    # ── 0. 캘린더 정책 노트 (월요일/금요일 특수 모드) ──────────────────────────
    _policy_note = ""
    try:
        from tele_quant.calendar_score_policy import format_policy_note
        _policy_note = format_policy_note()
    except Exception as _pe:
        log.debug("[advisor_4h] calendar policy note failed: %s", _pe)
...
def _assemble_advisory_message(
    ...
    # ① 헤더
    parts.append(f"📊 {market} 4H 퀀터멘탈 관찰 브리핑 — {kst_now} UTC\n")

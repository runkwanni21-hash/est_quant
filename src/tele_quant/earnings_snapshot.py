...
def fetch_earnings_snapshot(symbol: str, market: str = "") -> EarningsSnapshot:
    ...
    try:
        ...
        # ── Revenue ───────────────────────────────────────────────────────────
        # KR: store in 조 (1조 = 1e12 KRW); US: store in B (1B = 1e9 USD)
        rev = info.get("totalRevenue")
        rev_growth = info.get("revenueGrowth")
        if rev is not None:
            snap.revenue_actual_bn = rev / (1e12 if mkt == "KR" else 1e9)
...
def format_earnings_snapshot(snap: EarningsSnapshot) -> str:
    ...
    is_kr = snap.market == "KR"
    eps_unit = "원" if is_kr else "$"
    if snap.eps_actual is not None:
        if is_kr:
            lines.append(f"  • EPS(TTM): {snap.eps_actual:+,.0f}원")
        else:
    ...
    if snap.eps_estimate is not None:
        if is_kr:
            lines.append(f"  • EPS(Forward): {snap.eps_estimate:+,.0f}원")
        else:
    ...
    if snap.revenue_actual_bn is not None:
        if snap.market == "KR":
            rev_str = f"{snap.revenue_actual_bn:.1f}조"
        else:
            rev_str = f"${snap.revenue_actual_bn:.1f}B"

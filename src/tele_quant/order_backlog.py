...
# ── 단일 종목 수주잔고 헬퍼 (sector_intelligence용) ─────────────────────────

def get_backlog_for_symbol(
    symbol: str,
    store: Any = None,
    dart_api_key: str = "",
    ...
    """단일 종목 수주잔고 이벤트 조회 — sector_intelligence에서 사용."""
    from tele_quant.settings import Settings

    if not dart_api_key:
        try:  # noqa: SIM105
            dart_api_key = Settings().dart_api_key or ""
        except Exception:
            pass
    ...
    if store is not None:
        try:
            db_rows = store.recent_all_backlog_events(days=lookback_days)
            sym_rows = [r for r in db_rows if (r.get("symbol") or "") == symbol]
            if sym_rows:
                events: list[BacklogEvent] = []
                for r in sym_rows[:5]:
                    events.append(BacklogEvent(
                        ...
                        backlog_tier=str(r.get("backlog_tier") or ""),
    ...
    # DART 직접 조회
    if symbol.endswith((".KS", ".KQ")) and dart_api_key:
        try:
            return _fetch_dart_backlog_for_symbol(symbol, dart_api_key, lookback_days, timeout)
        except Exception:
            pass

    # Static registry
    ev = _static_backlog_event(symbol)
    return [ev] if ev else []


def format_backlog_summary(events: list[BacklogEvent], sector_id: str = "") -> str:
    """수주잔고 이벤트 → /분석용 짧은 요약 텍스트."""
    if not events:
        return ""

    sector_emoji = "🚢" if "shipbuilding" in sector_id else "🛡" if "defense" in sector_id else "📋"
    lines = [f"{sector_emoji} 수주잔고 체크:"]

    for ev in events[:3]:
        amt = ev.amount_ok_krw_display() if ev.amount_ok_krw else ev.amount_usd_display()
        nm = ev.report_nm[:50] if ev.report_nm else "공시 제목 미확인"
        parts = [f"  • {nm}"]
        if amt:
            parts[0] += f" ({amt})"
        if ev.counterparty:
            parts[0] += f" — {ev.counterparty[:30]}"
        if ev.completion_date:
            parts[0] += f" / 납기 {ev.completion_date[:7]}"
        lines.extend(parts)

    lines.append("  ⚠ 후판 가격·달러/원 환율·인건비 원가 리스크 확인 필요")
    lines.append("  체크포인트: DART 공시 신규 수주 + Clarkson 선가 방향")
    return "\n".join(lines)

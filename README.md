...
Windows 사용자는 복잡한 설치 과정 없이 다음 단계만으로 바로 시작할 수 있습니다.

1. **의존성 설치**: `install.bat` 파일을 더블 클릭합니다.
   - `uv`와 `Python 3.12`가 자동으로 설치되고 모든 필요한 패키지가 구성됩니다.
2. **실행 및 설정**: `run.bat` 파일을 더블 클릭합니다.
   - 처음 실행 시 `.env.local` 파일이 자동으로 생성되고 메모장이 열립니다.
   - 메모장에서 `OPENDART_API_KEY` 등 필요한 API 키를 입력하고 **저장**한 뒤 메모장을 닫으세요.
   - ⚠ **주의**: `.env.local` 파일은 개인 키가 포함되므로 절대 Git에 커밋하거나 타인에게 공유하지 마세요.
3. **텔레그램 인증**: 인증이 필요한 경우 `auth_telegram.bat` 파일을 실행하여 로그인을 완료합니다 (최초 1회).
4. **사용**: 대시보드가 자동으로 브라우저(`http://localhost:8765`)에서 열립니다.
...
```bash
# 1. 의존성 설치 (Python 3.11+ 필요)
uv sync --all-extras --link-mode copy

# 2. 환경 설정
# env.example을 복사하여 .env.local을 생성합니다. (env.template은 호환성을 위해 유지됨)
cp env.example .env.local
nano .env.local   # API 키 입력 (OPENDART_API_KEY 등)
...
### Windows 배치 파일 (Beginner-Proof)
- `install.bat`: `uv` 및 `Python 3.12` 자동 설치 + 의존성 구성
- `run.bat`: 자동 환경 설정(`.env.local` 생성) + 브라우저 실행 + 대시보드 구동
- `auth_telegram.bat`: 텔레그램 세션 인증(`tele-quant auth`) 실행
- `preview_kr.bat`: 한국 시장 브리핑 미리보기 실행

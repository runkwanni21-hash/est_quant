...
초보 사용자는 `실행.bat` 하나만 사용하면 됩니다.

1. GitHub에서 이 저장소를 `clone`하거나 ZIP으로 내려받습니다. 기본 브랜치는 `main_tele`입니다.
2. `실행.bat`을 더블클릭합니다.
3. 첫 실행이면 `data/private/`, `logs/`, `.env.local`이 자동 생성되고 메모장이 열립니다.
4. `.env.local`에 필요한 값만 입력하고 저장한 뒤 창을 닫습니다.
5. `실행.bat`을 다시 더블클릭합니다.
6. Python/uv/패키지가 자동 확인·설치되고, 대시보드가 브라우저로 열립니다.

포트 `8765`가 이미 사용 중이면 `launcher.py`가 `8766`, `8767`처럼 빈 포트를 찾아 실제 주소를 콘솔에 표시하고 그 주소를 브라우저로 엽니다. `실행.bat`은 브라우저 주소를 고정으로 열지 않습니다.
...
| 설정 | 추가 기능 |
|------|-----------|
| `TELEGRAM_BOT_TOKEN`, `TELEGRAM_BOT_TARGET_CHAT_ID` | 브리핑/알림 텔레그램 발송 |
| `TELEGRAM_API_ID`, `TELEGRAM_API_HASH`, `TELEGRAM_PHONE` | 텔레그램 채널 수집 |
| `OPENDART_API_KEY` | 한국 전자공시 OpenDART 수집 |
| `FINNHUB_API_KEY`, `FRED_API_KEY`, `ECOS_API_KEY` | 뉴스·매크로 보강 |

OpenDART 키 이름은 `DART_API_KEY`가 아니라 `OPENDART_API_KEY`입니다. 실제 주문, 자동매매, 브로커 연동, 실계좌 매매 기능은 포함하지 않습니다.
...
| 자동 실행 타이머 | **24개** systemd timer (WSL Ubuntu 전용) |
...
## 자동 실행 구조 — 24개 timer · 25개 service (총 49개 systemd 파일, WSL Ubuntu 전용)
...
| `tele-quant-cycle-monday-open` | 월요일 장 개시 전 |
| `tele-quant-cycle-weekday-4h` | 평일 4시간 통합 사이클 |
| `tele-quant-cycle-weekend-issue` | 주말 이슈 수집 |
| `tele-quant-cycle-sunday-review` | 일요일 주간 리뷰 |
| `tele-quant-cycle-surge-collector` | 장중 서지 수집 |
| `tele-quant-cycle-maintenance` | 정기 유지보수 |
| `tele-quant-bulk-refresh` | 유니버스 일괄 갱신 |
| `tele-quant-universe` | 유니버스 업데이트 |
...
├── tests/                           ← 2,479개 테스트 (pytest 통과 확인)
├── systemd/                         ← 49개 timer·service 파일 (WSL Ubuntu 전용)
├── docs/                            ← 발표자료·마이그레이션 가이드
├── data/                            ← DB·session (Git 제외)
├── env.example                      ← 일반 사용자 설정 템플릿
└── env.template                     ← 호환용 설정 템플릿
```
...
`실행.bat`을 더블클릭하세요. 첫 실행에는 `.env.local`을 만든 뒤 메모장을 열고 종료합니다. 값을 저장한 뒤 다시 `실행.bat`을 실행하면 패키지 설치와 대시보드 실행이 이어집니다.
...
nano .env.local   # 필요한 API 키만 입력

# 3. 첫 텔레그램 사용자 계정 인증 (선택, 채널 수집을 쓸 때만 1회)

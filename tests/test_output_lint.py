...
# ── Regression fixtures — new checks ─────────────────────────────────────────

FIXTURE_KR_PRICE = (
    Path(__file__).parent / "fixtures" / "output_lint" / "bad_stock_snapshot_kr_price_scale.txt"
)
FIXTURE_TICKER = (
    Path(__file__).parent / "fixtures" / "output_lint" / "bad_relation_ticker_zero_padding.txt"
)
FIXTURE_TONE = (
    Path(__file__).parent / "fixtures" / "output_lint" / "bad_trading_advisory_tone.txt"
)


def _run_lint_file(path: Path) -> subprocess.CompletedProcess[str]:
    cmd = ["uv", "run", "tele-quant", "output-lint", "--file", str(path)]
    return subprocess.run(
        cmd,
        capture_output=True,
        text=True,
...
def test_kr_price_scale_fixture_caught():
    """KR 현재가 1,000,000원 이상 비정상 스케일을 HIGH로 잡아야 한다."""
    result = _run_lint_file(FIXTURE_KR_PRICE)
    combined = result.stdout + result.stderr
    assert "HIGH" in combined or "스케일" in combined, (
        f"KR 가격 스케일 오류 미감지:\n{combined}"
    )


def test_bare_ticker_zero_padding_caught():
    """1~5자리 bare 숫자 티커 (17856, 42) 출력을 HIGH로 잡아야 한다."""
    result = _run_lint_file(FIXTURE_TICKER)
    combined = result.stdout + result.stderr
    assert "HIGH" in combined, f"bare 티커 미감지:\n{combined}"


def test_tone_fixture_catches_forbidden():
    """매수 권장, 매도 권장, 확정 수익 등 금지 표현을 HIGH로 잡아야 한다."""
    result = _run_lint_file(FIXTURE_TONE)
    combined = result.stdout + result.stderr
    assert "HIGH" in combined, f"금지 표현 미감지:\n{combined}"


def test_tone_fixture_catches_mae_su():
    result = _run_lint_file(FIXTURE_TONE)
    combined = result.stdout + result.stderr
    assert "매수 권장" in combined or "HIGH" in combined


def test_tone_fixture_catches_4h_advisory_tone():
    result = _run_lint_file(FIXTURE_TONE)
    combined = result.stdout + result.stderr
    assert "4H 매매 어드바이징" in combined or "HIGH" in combined

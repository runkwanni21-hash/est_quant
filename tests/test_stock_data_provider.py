...
def test_get_ohlcv_rate_limit_uses_fallback():
    df = _make_df()

    with (
        patch("yfinance.download", side_effect=RuntimeError("YFRateLimitError: Too Many Requests")),
        patch("tele_quant.stock_data_provider._fallback_ohlcv", return_value=(df, "finance-datareader")),
    ):
        result = sdp.get_ohlcv("NVDA", period="3mo", interval="1d")

    status = sdp.get_ohlcv_status("NVDA", period="3mo", interval="1d")
    assert result is not None
    assert status.status == "fallback"
    assert status.rate_limited is True
    assert status.source == "finance-datareader"


def test_get_ohlcv_rate_limit_skips_yahoo_during_cooldown():
    df = _make_df()

    with (
        patch("yfinance.download", side_effect=RuntimeError("Too Many Requests")),
        patch("tele_quant.stock_data_provider._fallback_ohlcv", return_value=(None, "")),
    ):
        sdp.get_ohlcv("FIRST", period="3mo", interval="1d")

    with (
        patch("yfinance.download") as mock_dl,
        patch("tele_quant.stock_data_provider._fallback_ohlcv", return_value=(df, "finnhub")),
    ):
        result = sdp.get_ohlcv("MSFT", period="3mo", interval="1d")

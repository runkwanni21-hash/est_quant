...
def _find_available_port(start_port: int, attempts: int = 20) -> int | None:
...
def main() -> None:
    ...
    if port is None:
        end_port = args.port + 19
        print(f"  [오류] {args.port}~{end_port} 포트 모두 사용 중입니다.")
        input("\n  [Enter] 키를 누르면 종료됩니다...")
        sys.exit(1)

    if port != args.port:
        print(f"  [경고] {args.port} 포트 사용 중 → {port} 포트로 변경")

    url = _browser_url(args.host, port)
    print(f"   실제 대시보드 주소: {url}")
    print(f"   사용 포트: {port}")
    print("   종료: Ctrl+C")
    print("  ============================================")
    print()
    ...
    if not args.no_browser:
        def _browser_opener() -> None:
            if _wait_for_server(port, timeout=25.0):
                ...
                print(f"  [경고] 서버 준비 확인 시간 초과. 직접 접속: {url}")
